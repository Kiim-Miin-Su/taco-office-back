...
describe('일정 조회/경로 → DTO → service 경계 (DB/인증은 통합 suite에서 검증)', () => {
  ...
  beforeEach(() => { role = 'ceo'; perms = {}; jest.clearAllMocks(); });
  ...
  it('전체 일정 범위는 CRUD가 아니라 관리자 화면 권한과 같은 결론을 쓴다', async () => {
    role = 'admin';
    perms = { canAdminPage: false, canCrudAll: true };
    await request(app.getHttpServer()).get('/schedule/occurrences').query({ ...range, teacherId: 12 }).expect(200);
    expect(list).toHaveBeenLastCalledWith(expect.objectContaining({ teacherId: 11 }));

    role = 'teacher';
    perms = { canAdminPage: true, canCrudAll: false };
    await request(app.getHttpServer()).get('/schedule/occurrences').query({ ...range, teacherId: 12 }).expect(200);
    expect(list).toHaveBeenLastCalledWith(expect.objectContaining({ teacherId: 12 }));
  });
  ...
  it('SER가 있어도 실제 회차가 없는 tracking 조회는 OCCURRENCE_NOT_FOUND다', async () => {
    ...
    expect(res.body).toMatchObject({ code: 'OCCURRENCE_NOT_FOUND' });
    expect(tracking).toHaveBeenCalledWith(1, date, true);
  });
  const mutations = [
